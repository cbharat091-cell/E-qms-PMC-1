...
async function startServer() {
  ...
  // Serve uploaded files statically so they can be downloaded back
  ...
  app.get("/api/data", (req, res) => {
    try {
      ...
      return res.json({ users: [], documents: [], auditLogs: [] });
    } catch (err) {
  ...
  app.post("/api/sync", (req, res) => {
    try {
      const { users, documents, auditLogs } = req.body;
      const model = {
        users: users || [],
        documents: documents || [],
        auditLogs: auditLogs || []
  ...
  if (process.env.NODE_ENV !== "production") {
    try {
      ...
      console.warn("Vite development module not loaded, serving static fallback from root...");
      app.use(express.static(__dirname));
      app.use((req, res, next) => {
        const isApiOrUpload = req.path.startsWith('/api') || req.path.startsWith('/uploads') || req.path.startsWith('/vault');
        const isStaticAsset = /\.[a-zA-Z0-9]+$/.test(req.path);
        const acceptsHtml = req.headers.accept && req.headers.accept.includes('text/html');

        if (req.method === 'GET' && !isApiOrUpload) {
    ...
    // Serve static files from the current directory (__dirname) for LAN deployment
    app.use(express.static(__dirname));
    app.use((req, res, next) => {
      const isApiOrUpload = req.path.startsWith('/api') || req.path.startsWith('/uploads') || req.path.startsWith('/vault');
      const isStaticAsset = /\.[a-zA-Z0-9]+$/.test(req.path);
